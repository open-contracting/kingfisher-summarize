...
DECLARE
    query text;
BEGIN
    query := $query$ ALTER TABLE release_summary
        ADD CONSTRAINT release_summary_release_summary_with_checks_fk FOREIGN KEY (id) REFERENCES release_summary_with_checks (id) NOT valid;
    ALTER TABLE release_summary
        ADD CONSTRAINT release_summary_release_summary_with_data_fk FOREIGN KEY (id) REFERENCES release_summary_with_data (id) NOT valid;
    $query$;
    EXECUTE query;
EXCEPTION
    WHEN wrong_object_type THEN
