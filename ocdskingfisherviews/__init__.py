...
from ocdskingfisherviews.db import commit, pluck, set_search_path
...
def do_field_counts(cursor, schema, remove=False, threads=1):
    ...
    def _run_collection(collection):
        ...
        if results:
            cursor.execute('INSERT INTO field_counts_tmp VALUES (%s, %s, %s, %s, %s, %s)', *results)
            commit()
