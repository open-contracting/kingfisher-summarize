...
@click.argument("note")
@click.option(
    "--name", callback=validate_name, help=f'A custom name for the SQL schema ("{SCHEMA_PREFIX}" will be prepended).'
)
@click.option("--tables-only", is_flag=True, help="Create SQL tables instead of SQL views.")
@click.option(
    "--field-counts/--no-field-counts",
    "field_counts_option",
    default=True,
    help="Whether to create the field_counts table (default true).",
)
@click.option(
    "--field-lists/--no-field-lists",
    "field_lists_option",
    default=False,
    help="Whether to add a field_list column to all summary tables (default false).",
)
@click.option("--skip", multiple=True, help="Any SQL files to skip. Dependent files and final files will be skipped.")
@click.option(
    "--filter",
    "filters",
    nargs=2,
...
def add(
    ...
    logger.info(
        "Arguments: collections=%s note=%s name=%s tables_only=%s filters=%s filters_sql_json_path=%s",
        collections,
        note,
        name,
        tables_only,
    ...
    db.execute(
        "INSERT INTO note (note, created_at) VALUES (%(note)s, %(created_at)s)",
...
def summary_tables(name, *, tables_only=False, skip=(), where_fragment=None):
    ...
    files = {
        directory: sql_files(directory, tables_only=tables_only, where_fragment=where_fragment)
...
def field_counts(name):
    ...
    db.execute(
        "COMMENT ON COLUMN field_counts.collection_id IS '``id`` from the Kingfisher Process ``collection`` table'"
    )
    db.execute(
        "COMMENT ON COLUMN field_counts.release_type IS "
        """'Either "release", "compiled_release", "record" or "embedded_release"'"""
    )
    db.execute("COMMENT ON COLUMN field_counts.path IS 'JSON path of the field, excluding array indices'")
    db.execute(
        "COMMENT ON COLUMN field_counts.object_property IS "
        "'Number of occurrences of the field, across all array entries and all releases'"
    )
    db.execute(
        "COMMENT ON COLUMN field_counts.array_count IS 'If the field is an array, "
        "cumulative length of its occurrences, across all array entries and all releases'"
    )
    db.execute("COMMENT ON COLUMN field_counts.distinct_releases IS 'Number of releases in which the field occurs'")
    db.commit()
