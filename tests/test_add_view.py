...
from ocdskingfisherviews.db import fetchall, schema_exists
from tests import ADD_VIEW_TABLES, REFRESH_VIEWS_TABLES, fixture, get_tables
...
def test_command(caplog):
    with fixture() as result:
        ...
        assert fetchall('SELECT * FROM view_data_collection_1.selected_collections') == [(1,)]
        assert fetchall('SELECT id, note FROM view_data_collection_1.note') == [(1, 'Default')]
...
def test_command_multiple(caplog):
    with fixture(collections='1,2') as result:
        ...
        assert fetchall('SELECT * FROM view_data_collection_1_2.selected_collections') == [(1,), (2,)]
        assert fetchall('SELECT id, note FROM view_data_collection_1_2.note') == [(1, 'Default')]
