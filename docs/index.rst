...
Once Kingfisher Views is :doc:`installed<get-started>`, use the :ref:`add-view` command to create schemas that summarize one or more collections of your choice. (This command might take a long time to run. You might want to run it in a terminal multiplexer like ``tmux``.)
...
As documented in the :ref:`add-view` command, the schema you created has a name starting with ``view_data_``, like ``view_data_collection_123`` or ``view_data_collection_4_5_6`` or ``view_data_the_name``. To learn more about each table in the schema, refer to the :doc:`database`.
...
Instead of typing the schema name every time, you can set PostgreSQL's `search_path <https://www.postgresql.org/docs/current/runtime-config-client.html#GUC-SEARCH-PATH>`__ to a comma-separated list of schemas in which to search for tables. For example, if you want to query both a Kingfisher Views schema and Kingfisher Process' tables, run this statement first:
...
Once you no longer need the schema, delete it using the :ref:`delete-view` command to free up disk space. (You can re-create it at any time using the :ref:`add-view` command.)

List all schemas
~~~~~~~~~~~~~~~~

To get a list of schemas created by yourself or others, use the :ref:`list-views` command. It reports:
